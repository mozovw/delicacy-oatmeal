...
package com.delicacy.oatmeal.guava.demo;
   ...
   //Guava  0.5的意思是 1秒中0.5次的操作，2秒1次的操作  从速度来限流，从每秒中能够执行的次数来
    ...
    private static void testSemaphore(){
        try {
            ...
            sem.release();
            System.out.println(currentThread().getName()+" release the semephore..other thread can get and do task");
        }
    }
    ...
    public static void runTestSemaphore(){
        ...
        IntStream.range(0,10).forEach((i)->{
            //RateLimiterDemo::testLimiter 这种写法是创建一个线程
            service.submit(RateLimiterDemo::testSemaphore);
        });
    }
    ...
    public static void runTestLimiter(){
        ...
        IntStream.range(0,10).forEach((i)->{
            //RateLimiterDemo::testLimiter 这种写法是创建一个线程
            service.submit(RateLimiterDemo::testLimiter);
        });
    }
